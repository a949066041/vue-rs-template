/* eslint-disable */
/* prettier-ignore */
// oxfmt-ignore
// @ts-nocheck
// noinspection ES6UnusedImports
// Generated by vue-router. !! DO NOT MODIFY THIS FILE !!
// It's recommended to commit this file.
// Make sure to add this file to your tsconfig.json file as an "includes" or "files" entry.

import type {
  RouteRecordInfo,
  ParamValue,
  ParamValueOneOrMore,
  ParamValueZeroOrMore,
  ParamValueZeroOrOne,
} from 'vue-router'
import type {
  _ExtractParamParserType,
} from 'vue-router/experimental'

declare module 'vue-router' {
  interface TypesConfig {
    ParamParsers:
      | never
    RouteNamedMap: import('vue-router/auto-routes').RouteNamedMap
  }
}

declare module 'vue-router/auto-routes' {
  /**
   * Route name map generated by vue-router
   */
  export interface RouteNamedMap {
    '/(auth)/page': RouteRecordInfo<
      '/(auth)/page',
      '/page',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/': RouteRecordInfo<
      '/',
      '/',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/[...path]': RouteRecordInfo<
      '/[...path]',
      '/:path(.*)',
      { path: ParamValue<true> },
      { path: ParamValue<false> },
      | never
    >,
    '/about/': RouteRecordInfo<
      '/about/',
      '/about',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/about/[id]': RouteRecordInfo<
      '/about/[id]',
      '/about/:id',
      { id: ParamValue<true> },
      { id: ParamValue<false> },
      | never
    >,
    '/about/[value]/': RouteRecordInfo<
      '/about/[value]/',
      '/about/:value',
      { value: ParamValue<true> },
      { value: ParamValue<false> },
      | never
    >,
    '/about/[value]/create': RouteRecordInfo<
      '/about/[value]/create',
      '/about/:value/create',
      { value: ParamValue<true> },
      { value: ParamValue<false> },
      | never
    >,
    '/about/name/': RouteRecordInfo<
      '/about/name/',
      '/about/name',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/about/name/[name]': RouteRecordInfo<
      '/about/name/[name]',
      '/about/name/:name',
      { name: ParamValue<true> },
      { name: ParamValue<false> },
      | never
    >,
    '/auth': RouteRecordInfo<
      '/auth',
      '/auth',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/event/326': RouteRecordInfo<
      '/event/326',
      '/event/326',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/n': RouteRecordInfo<
      '/n',
      '/n',
      Record<never, never>,
      Record<never, never>,
      | '/n/'
      | '/n/value-[[more]]+/'
    >,
    '/n/': RouteRecordInfo<
      '/n/',
      '/n',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/n/value-[[more]]+/': RouteRecordInfo<
      '/n/value-[[more]]+/',
      '/n/value-:more*',
      { more?: ParamValueZeroOrMore<true> },
      { more?: ParamValueZeroOrMore<false> },
      | never
    >,
    '/nesting': RouteRecordInfo<
      '/nesting',
      '/nesting',
      Record<never, never>,
      Record<never, never>,
      | '/nesting/'
      | '/nesting/nesting2'
      | '/nesting/nesting2/test2'
      | '/nesting/nesting2/test3'
    >,
    '/nesting/': RouteRecordInfo<
      '/nesting/',
      '/nesting',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/nesting/nesting2': RouteRecordInfo<
      '/nesting/nesting2',
      '/nesting/nesting2',
      Record<never, never>,
      Record<never, never>,
      | '/nesting/nesting2/test2'
      | '/nesting/nesting2/test3'
    >,
    '/nesting/nesting2/test2': RouteRecordInfo<
      '/nesting/nesting2/test2',
      '/nesting/nesting2/test2',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/nesting/nesting2/test3': RouteRecordInfo<
      '/nesting/nesting2/test3',
      '/nesting/nesting2/test3',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/query': RouteRecordInfo<
      '/query',
      '/query',
      Record<never, never>,
      Record<never, never>,
      | '/query/'
      | '/query/[id]'
      | '/query/action'
      | '/query/page'
    >,
    '/query/': RouteRecordInfo<
      '/query/',
      '/query',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/query/[id]': RouteRecordInfo<
      '/query/[id]',
      '/query/:id',
      { id: ParamValue<true> },
      { id: ParamValue<false> },
      | never
    >,
    '/query/action': RouteRecordInfo<
      '/query/action',
      '/query/action',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/query/page': RouteRecordInfo<
      '/query/page',
      '/query/page',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/store': RouteRecordInfo<
      '/store',
      '/store',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/user': RouteRecordInfo<
      '/user',
      '/user',
      Record<never, never>,
      Record<never, never>,
      | '/user/'
      | '/user/[id]'
      | '/user/create.[id]'
      | '/user/page'
    >,
    '/user/': RouteRecordInfo<
      '/user/',
      '/user',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
    '/user/[id]': RouteRecordInfo<
      '/user/[id]',
      '/user/:id',
      { id: ParamValue<true> },
      { id: ParamValue<false> },
      | never
    >,
    '/user/create.[id]': RouteRecordInfo<
      '/user/create.[id]',
      '/user/create/:id',
      { id: ParamValue<true> },
      { id: ParamValue<false> },
      | never
    >,
    '/user/page': RouteRecordInfo<
      '/user/page',
      '/user/page',
      Record<never, never>,
      Record<never, never>,
      | never
    >,
  }

  /**
   * Route file to route info map by vue-router.
   * Used by the \`sfc-typed-router\` Volar plugin to automatically type \`useRoute()\`.
   *
   * Each key is a file path relative to the project root with 2 properties:
   * - routes: union of route names of the possible routes when in this page (passed to useRoute<...>())
   * - views: names of nested views (can be passed to <RouterView name="...">)
   *
   * @internal
   */
  export interface _RouteFileInfoMap {
    'src/pages/(auth)/page.vue': {
      routes:
        | '/(auth)/page'
      views:
        | never
    }
    'src/pages/index.vue': {
      routes:
        | '/'
      views:
        | never
    }
    'src/pages/[...path].vue': {
      routes:
        | '/[...path]'
      views:
        | never
    }
    'src/pages/about/index.vue': {
      routes:
        | '/about/'
      views:
        | never
    }
    'src/pages/about/[id].vue': {
      routes:
        | '/about/[id]'
      views:
        | never
    }
    'src/pages/about/[value]/index.vue': {
      routes:
        | '/about/[value]/'
      views:
        | never
    }
    'src/pages/about/[value]/create.vue': {
      routes:
        | '/about/[value]/create'
      views:
        | never
    }
    'src/pages/about/name/index.vue': {
      routes:
        | '/about/name/'
      views:
        | never
    }
    'src/pages/about/name/[name].vue': {
      routes:
        | '/about/name/[name]'
      views:
        | never
    }
    'src/pages/auth.vue': {
      routes:
        | '/auth'
      views:
        | never
    }
    'src/pages/event/326.vue': {
      routes:
        | '/event/326'
      views:
        | never
    }
    'src/pages/n.vue': {
      routes:
        | '/n'
        | '/n/'
        | '/n/value-[[more]]+/'
      views:
        | 'default'
    }
    'src/pages/n/index.vue': {
      routes:
        | '/n/'
      views:
        | never
    }
    'src/pages/n/value-[[more]]+/index.vue': {
      routes:
        | '/n/value-[[more]]+/'
      views:
        | never
    }
    'src/pages/nesting.vue': {
      routes:
        | '/nesting'
        | '/nesting/'
        | '/nesting/nesting2'
        | '/nesting/nesting2/test2'
        | '/nesting/nesting2/test3'
      views:
        | 'default'
    }
    'src/pages/nesting/index.vue': {
      routes:
        | '/nesting/'
      views:
        | never
    }
    'src/pages/nesting/nesting2.vue': {
      routes:
        | '/nesting/nesting2'
        | '/nesting/nesting2/test2'
        | '/nesting/nesting2/test3'
      views:
        | 'default'
    }
    'src/pages/nesting/nesting2/test2.vue': {
      routes:
        | '/nesting/nesting2/test2'
      views:
        | never
    }
    'src/pages/nesting/nesting2/test3.vue': {
      routes:
        | '/nesting/nesting2/test3'
      views:
        | never
    }
    'src/pages/query.vue': {
      routes:
        | '/query'
        | '/query/'
        | '/query/[id]'
        | '/query/action'
        | '/query/page'
      views:
        | 'default'
    }
    'src/pages/query/index.vue': {
      routes:
        | '/query/'
      views:
        | never
    }
    'src/pages/query/[id].vue': {
      routes:
        | '/query/[id]'
      views:
        | never
    }
    'src/pages/query/action.vue': {
      routes:
        | '/query/action'
      views:
        | never
    }
    'src/pages/query/page.vue': {
      routes:
        | '/query/page'
      views:
        | never
    }
    'src/pages/store.vue': {
      routes:
        | '/store'
      views:
        | never
    }
    'src/pages/user.vue': {
      routes:
        | '/user'
        | '/user/'
        | '/user/[id]'
        | '/user/create.[id]'
        | '/user/page'
      views:
        | 'default'
    }
    'src/pages/user/index.vue': {
      routes:
        | '/user/'
      views:
        | never
    }
    'src/pages/user/[id].vue': {
      routes:
        | '/user/[id]'
      views:
        | never
    }
    'src/pages/user/create.[id].vue': {
      routes:
        | '/user/create.[id]'
      views:
        | never
    }
    'src/pages/user/page.vue': {
      routes:
        | '/user/page'
      views:
        | never
    }
  }

  /**
   * Get a union of possible route names in a certain route component file.
   * Used by the \`sfc-typed-router\` Volar plugin to automatically type \`useRoute()\`.
   *
   * @internal
   */
  export type _RouteNamesForFilePath<FilePath extends string> =
    _RouteFileInfoMap extends Record<FilePath, infer Info>
      ? Info['routes']
      : keyof RouteNamedMap
}

export {}
